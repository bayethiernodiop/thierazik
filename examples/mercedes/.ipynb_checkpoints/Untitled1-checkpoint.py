...
class HyperOptimizer(object):
    def __init__(self, search_space,X,y,scoring,trial_file,trial_step=1, trial_initial_step=1,
     debug=False, n_split=3, neg_pos_scoring=1, preprocess_steps = None):
        ...
        self.debug = debug
        self.trial_file = os.path.join(thierazik.config["PATH"],"hp_trials",trial_file)
        self.X = X
        self.y=y
        self.cv_inner = KFold(
            n_splits=n_split, 
    ...
    def run_trials(self):
        os.makedirs(self.trial_file, exist_ok=True)

        try:  # try to load an already saved trials object, and increase the max
            # use data path for this project
            hypopt_trials = pickle.load(open(self.trial_file, "rb"))
        ...
        with open(self.trial_file, mode="wb") as f:
...
# + {"Collapsed": "false"}
optimizer = HyperOptimizer(search_space=space,X,y,scoring="accuracy",trial_file,trial_step=1, trial_initial_step=1,
     debug=False, n_split=3, neg_pos_scoring=1, preprocess_steps = None)
